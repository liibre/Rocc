...
These functions download the IPT files from FB2020 to a temporary file in the computer, using package __finch__. This download can be __very slow__ the first time and when in disk the database will access the cached files instead of downloading them. 
...
```{r}
length(unique(intersect(caatinga$id, pb$id)))
```
...
```{r}
PE_and_caatinga_and_end <- search_flora(domain = "Caatinga",
                            state = "PE",
                            endemism = TRUE)
head(PE_and_caatinga_and_end)
dim(PE_and_caatinga_and_end)
```
